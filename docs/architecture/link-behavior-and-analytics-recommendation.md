...
The temporary palette controls store selection and visibility in local storage (`dfn-palette` and `dfn-palette-hidden`). They do not add or interpret URL settings. Links preserve authored URLs without propagating palette parameters. A small Colours button restores hidden controls. This preference stays in the current browser and origin; it is not included in shared URLs. Palette-specific tests were removed because this is temporary design tooling.

Header/footer homepage anchors still point home from content pages, except local `#dh-contact`. Grouped navigation order, disclosure controls and drawer-close behavior remain intact. The unconditional ↗ was changed to →. Current-page matching is shared and excludes fragments/outbound destinations. Content pageview trackers remount when the slug changes so client transitions retain a named pageview for each page visit.
...
Validation includes lint, TypeScript, the production build and the remaining navigation/analytics tests. Tests cover URL origins/forms, renderer choice, refs/downloads, current-page state, cancellation, modified/keyboard clicks and exact legacy event name/properties/coverage. DOM tests mock only Next's renderer when testing selection; they do not claim to test Next's own router internals.

The original browser checks covered About→Privacy, Back, keyboard activation and mobile drawer navigation. Public markup contained no forced new-tab links. The shared module uses Next's native modifier handling rather than reimplementing it. Palette behavior is deliberately kept as temporary browser-local UI rather than part of the link contract.
