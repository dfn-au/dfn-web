export async function capturePageSnapshot(
	...
	return page.evaluate(
		({ rootSelector, urlPathPrefix }) => {
			...
			function normalizeStyleValue(value, documentOrigin) {
				const normalizedValue = value.replaceAll(documentOrigin, "<origin>");
				if (!urlPathPrefix) return normalizedValue;

				return normalizedValue.replaceAll(
					`<origin>${urlPathPrefix}`,
					"<origin>",
				);
			}
		...
		{ rootSelector: selector, urlPathPrefix },
	);
}
